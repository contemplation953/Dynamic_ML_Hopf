...
##Approximate range of T_L and polynomial coefficients
...
res = DiffEqFlux.sciml_train(loss1, θ, ADAM(0.1); maxiters=8000,cb=callbackf)
θ1_ = res.minimizer
...
res2 = DiffEqFlux.sciml_train(loss_norm_param, θ, Adam(0.001); maxiters=2500,cb=callbackf)
...
θ = vec(pol_coeffs);
res1 = DiffEqFlux.sciml_train(loss_pol_coeffs, θ, Adam(0.01); maxiters=1500,cb=callbackf)
θ_ = res1.minimizer
pol_coeffs = θ_[1:end]
...
res2 = DiffEqFlux.sciml_train(loss_nn, θn, ADAM(0.01); maxiters=5000,cb=callbackf)
θn_ = res2.minimizer
