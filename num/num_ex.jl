...
AA = load("./num/inputdata/AA.jld","AA")
...
##Training approximate range of T_L and the polynomial coefficients

function getPr(norm_bif_params)
    train_t_series = Differentiate_Function.generate_LCO_ts(normal_form, norm_bif_params, nh, u0, tl, tol, stol, st, st2, 1, 2)
    train = [
       train_t_series[i][:,end-θ_l+1:end] for i in 1:length(train_t_series)
    ]
...
iter = 0
log_io = open("./num/outputdata/log.log", "a")
function callbackf(t, l)
    ...
    write(log_io, "train $iter,error $(l),time $(time())\n")
    return false
end

#Pre-calculated normal data
ts_dict = load("./num/inputdata/ts.jld","ts")

init_V₀ = 98
#T_L
θₜ = vec([1.0 0.0; 0.0 1.0])
#Finding the approximate range of bifurcation parameters for the normal form
...
res = DiffEqFlux.sciml_train(loss1, θ, ADAM(0.1); maxiters=7000,cb=callbackf)
θ1_ = res.minimizer
save("./num/outputdata/θ1_.jld","θ1_",θ1_);
# θ1_ = load("./num/outputdata/θ1_.jld","θ1_");
θₜ = θ1_[1:4]
save("./num/outputdata/θₜ.jld","θₜ",θₜ);
...
res2 = DiffEqFlux.sciml_train(loss_norm_param, θ, Adam(0.0001); maxiters=2000,cb=callbackf)
norm_bif_params = res2.minimizer
save("./num/outputdata/norm_bif_params.jld","norm_bif_params",norm_bif_params);
...
θ = vec(pol_coeffs);
res1 = DiffEqFlux.sciml_train(loss_pol_coeffs, θ, Adam(0.01); maxiters=1000,cb=callbackf)
θ_ = res1.minimizer
pol_coeffs = θ_[1:end]
save("./num/outputdata/pol_coeffs.jld","pol_coeffs",pol_coeffs);
...
res2 = DiffEqFlux.sciml_train(loss_nn, θn, ADAM(0.01); maxiters=3000,cb=callbackf)
θn_ = res2.minimizer
save("./num/outputdata/θn_.jld","θn_",θn_);
