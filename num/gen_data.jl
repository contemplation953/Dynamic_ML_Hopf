...
function generate_data(vel_l, Vel, nh, u0, tol, stol)
...
dat = generate_data(vel_l, Vel, nh, u0, tol, stol)
save("./num/inputdata/AA.jld","AA",dat.data)
save("./num/inputdata/t_series.jld","t_series",dat.ts)

t_s = dat.ts
plot(t_s[1][1,:], t_s[1][2,:], color=:blue, linewidth=1.5, label="Limit cycle",xlabel=L"y_{w1}(mm)",ylabel = L"\varphi_{w1}")
savefig("./num/inputdata/bogie_phase.pdf")


vars = matread("./num/inputdata/bif.mat")
bifvalue = get(vars, "res1", 1)
v_num = size(bifvalue)[1]
step_len = 0.05
for i = 1:v_num
    v = V₀ + (i-1)*step_len
    if i == 1
        scatter(ones(v_num)*v,bifvalue[i,:]*1000,color=:blue,markerstrokecolor=:blue, msize=1, label="Local maximum",xlabel="v(m/s)",ylabel=L"y_{w1}(mm)")
    else
        scatter!(ones(v_num)*v,bifvalue[i,:]*1000,color=:blue,markerstrokecolor=:blue, label="", msize=0.5)
    end
end
savefig("./num/inputdata/bogie_bif.pdf")
